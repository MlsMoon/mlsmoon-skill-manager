...
| `-catalog` | 读真实 `catalog/skills.json`：id 唯一、companion 不在 `skills`、`spine-gpu-skinning` 只有 unity、psd/3d 为 all、公开清单没有 `source: lan` / 内网 host、`installPath` 不含 `..`；`packages` 与 `plugins` 同样校验 | 对照 `mlsmoon-catalog` |
